...
"""
CREDITS TO: https://github.com/megadose/holehe
"""
...
def print_result(data,email,start_time,websites):
    ...
    for results in data:
        if results["rateLimit"]:
            ...
            websiteprint = "[+] " + results["domain"] + toprint         
            resoult.append(results["domain"] + toprint)
...
async def maincore(email):
    ...
    await client.aclose()
    # Print the result
    return print_result(out,email,start_time,websites)

def holohe(target):
    return trio.run(maincore, target)
